...
// 	// 检查颜色
// 	Vec3i color = {0, 0, 0};
// 	double x_list[] = {-1.2,-1.1, -1., 1., 1.1, 1.2};
// 	double y_list[] = {-0.8,-0.5, -0.3, 0., 0.3, 0.5, 0.8};
// 	for(double x: x_list) {
// 		for(double y: y_list) {
// 			auto pos =
...
std::pair<Light, LightCriterion> Light::from_contour(const Contour& contour,
	...
	// 检查颜色
	Vec3i color = {0, 0, 0}; // BGR
	for(cv::Point cnt : contour) {
		color[0] += img.at<Vec3b>(cnt.y, cnt.x)[0];
		color[2] += img.at<Vec3b>(cnt.y, cnt.x)[2];
	}
