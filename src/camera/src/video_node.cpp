...
public:
    StreamNode() : Node("stream_node") {
        ...
        timer_ = this->create_wall_timer(
            1ms, std::bind(&StreamNode::publish, this));
...
private:
    ...
    void openVideoFile(const std::string &file_path) {
        ...
        if (!cap_.isOpened()) {
            RCLCPP_ERROR(this->get_logger(), "Unable to open video file: %s", file_path.c_str());
        } else {
            RCLCPP_INFO(this->get_logger(), "Video file opened successfully: %s", file_path.c_str());
        }
    }
